...
class BackdoorDetector:
    ...
    # Scanning Logic
    def scan(self, adapter_path: str) -> Dict:
        ...
        try:
            ...
            report = self.analyzer.analyze(matrices)
            return {
    ...
    def calibrate(self, poison_paths: List[str], benign_paths: List[str]):
        ...
        for is_poison, paths in [(1, poison_paths), (0, benign_paths)]:
            for p in paths:
                try:
                    mats = self.extract_delta_w(p)
                    # Get raw z-scores for optimization
                    # We access analyzer's shared math to keep logic consistent
        ...
        new_weights = np.abs(clf.coef_[0]) / np.sum(np.abs(clf.coef_[0]))
        ...
        scores = [self.scan(p)['score'] for p in poison_paths + benign_paths]
